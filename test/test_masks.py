...
def test_card_masking() -> None:
    ...
    assert mask_card_number("1234") == "Ошибка: номер карты должен содержать минимум 10 цифр"


def test_account_masking() -> None:
    assert mask_account_number("73654108430135874305") == "**4305"
    assert mask_account_number("123") == "Ошибка: номер счёта должен содержать минимум 4 цифры"
...
def test_get_mask_card_number_invalid(invalid_card_numbers: List[str]) -> None:
    for number in invalid_card_numbers:
        assert "Ошибка" in mask_card_number(number)
...
def test_get_mask_account_invalid(invalid_account_numbers: List[str]) -> None:
    for number in invalid_account_numbers:
        assert "Ошибка" in mask_account_number(number)
